...
class TestTranslateModelIdForLitellm:
    ...
    def test_preserves_custom_openai_compatible_model(self):
    ...
    def test_translates_bare_anthropic_model_without_prefix(self):
        """Translates bare Anthropic model without provider prefix."""
        result = translate_model_id_for_litellm("claude-3.5-sonnet")
        assert result == "claude-3-5-sonnet"

    def test_preserves_bare_non_anthropic_model_with_dots(self):
        """Preserves bare non-Anthropic model IDs with dots."""
        result = translate_model_id_for_litellm("Qwen3.5-9B")
        assert result == "Qwen3.5-9B"

    def test_preserves_bare_openai_model_with_dots(self):
        """Preserves bare OpenAI model IDs with dots."""
