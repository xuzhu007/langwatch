...
vi.mock("~/hooks/useDrawer", () => ({
  useDrawer: () => ({ openDrawer: drawerMock.open }),
}));
  ...
  render(
    <ChakraProvider value={defaultSystem}>
      <BulkActionBar
        ...
        traceTimeRange={{ from: 1_000, to: 2_000 }}
...
describe("BulkActionBar Add to context", () => {
  ...
  describe("given explicit trace selection", () => {
    it("passes trace IDs and the active time range to the dataset drawer", async () => {
      useSelectionStore.getState().setMany(["t1", "t2"], true);
      renderBar();
      ...
      await waitFor(() => {
        expect(drawerMock.open).toHaveBeenCalledWith("addDatasetRecord", {
