    ...
    return await this.tracer.withActiveSpan(
      ...
      async () => {
        ...
        const uniqueTraceIds = Array.from(
          new Set(traceIds.filter((traceId) => traceId.length > 0)),
        );
        ...
        try {
          ...
          for (
            let start = 0;
            start < uniqueTraceIds.length;
            start += batchSize
          ) {
            const batch = uniqueTraceIds.slice(start, start + batchSize);
            const batchResult = await this.fetchTracesWithSpansJoined(
              projectId,
              batch,
              occurredAt,
              needsSpanRead,
          ...
          if (!includeSpans) {
            return new Map(
              summaryRows.map((row) => [
                row.ts_TraceId,
                {
                  summary: this.rowToTraceSummaryData(row),
                  spans: [],
                },
              ]),
            );
