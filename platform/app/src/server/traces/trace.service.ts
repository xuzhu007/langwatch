    ...
    return this.tracer.withActiveSpan(
      ...
      async () => {
        const traces = await this.clickHouseService.getTracesWithSpans(
          ...
          {
            resolveBlobs: opts?.full,
            ...(opts?.includeSpans !== undefined
              ? { includeSpans: opts.includeSpans }
              : {}),
          },
        );
        // includeSpans === false 时跳过 coding-agent 富化（无 span 可补），
        // 其余路径与上游一致：先富化，再按需叠加 reviewer 修正。
        const enriched =
          opts?.includeSpans === false
            ? traces
            : await this.enrichCodingAgentTraces(projectId, traces);
